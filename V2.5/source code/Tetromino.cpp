...
namespace Tetromino
{
	...
	{
		return (c == 'I' || c == 'O' || c == 'T' || c == 'J' || c == 'L' || c == 'S' || c == 'Z' || c == 'Q');
	}

	/* GETTERS */
	unsigned int Tetromino::getReturnOriginCounter() { return returnOriginCounter;}
	char Tetromino::getLetter() const { return letter; }
    ...
    void Tetromino::setTargetCoordinates(int _targetX, int _targetY) 
	{	
        targetY = _targetY;
        targetX = _targetX; 
    }
	...
	Tetromino::Tetromino(shapes type)
	{
		...
		setShape(type);
        for(int i=0; i<4;i++)
            letterCoordinatesOnBoard.push_back({0,0});
	}
