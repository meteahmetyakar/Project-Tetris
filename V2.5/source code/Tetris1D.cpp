...
namespace Tetris1D
{
    ...
    unsigned int Tetris1D::getRowSize(){return row;}
	unsigned int Tetris1D::getColSize(){return col;}
    int Tetris1D::getCreationAreaHeight(){return creationAreaHeight;}

    void Tetris1D::readFromFile()
	{
		ifstream myfile("Tetris1D.txt");
		string line;
		if(myfile.is_open())
		{
			getline(myfile, line);
			...
			delete [] gameArea;
			gameArea = new char[row*col];

			for(int i=0; i<row; i++)
			{
				for(int j=0; j<col; j++)
					myfile.get(gameArea[i*col + j]);
    ...
    void Tetris1D::writeToFile()
	{
		std::ofstream myfile("Tetris1D.txt");
		if(myfile.is_open())
		{
			myfile << row << "\n";
			myfile << col << "\n";
    ...
    bool Tetris1D::animate(Tetromino::Tetromino &tetro)
    {
        {
		...
		{
			Tetris1D::AbstractTetris::lastMovement = make_pair(-1,-1);
			throw -1;
		}
    ...
    bool Tetris1D::move(Tetromino::Tetromino &tetro, Tetromino::route direction)
    { 
        int xStep = 0, yStep = 0;
		...
		for(auto element : coordinates)
		{
			...
			//checking collision
			if(xOfelement+xStep < 0 || xOfelement+xStep >= this->getColSize() || yOfelement+yStep < 0 || yOfelement+yStep >= this->getRowSize() || this->gameArea[(yOfelement+yStep)*col + xOfelement+xStep] != ' ')
			{
				this->gameArea = tempBoard;
				for(int i=0; i<row*col; i++)
					gameArea[i] = tempBoard[i];
		...
		for(auto element : coordinates)
		{
			...
			this->gameArea[(yOfelement+yStep)*col + xOfelement+xStep] = tetro.getLetter();
		...
		Tetris1D::AbstractTetris::moves++;
        lMove = std::make_pair(tetro.letterCoordinatesOnBoard[0][0], tetro.letterCoordinatesOnBoard[0][1]);
		return true;
        
    }

    bool Tetris1D::fitRow(Tetromino::Tetromino &tetro)
    {
    ...
    std::pair<int,int> Tetris1D::lastMove(){return lMove;}

    Tetris1D& Tetris1D::operator+=(Tetromino::Tetromino& tetro)
    {
		...
		{
			gameArea[(startYIndex+(tetroLetterIndexes[i][0]-referenceY))*col + startXIndex+(tetroLetterIndexes[i][1]-referenceX)] = tetro.getLetter();
    ...
    void Tetris1D::clearIndexes(vector<vector<int>> indexes)
	{
		for(auto element : indexes)
			gameArea[element[0]*col + element[1]] = ' ';
			//gameArea[element[0]][element[1]] = ' ' ;
	}


    //constructor for define the map.
	Tetris1D::Tetris1D(unsigned int areaRow, unsigned int areaCol) : row{areaRow}, col{areaCol} 
	{
